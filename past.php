...
<body>
    <div class="main-block">
        <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
            <h1 align="center">Past Event</h1>
            <div class="info">
                <p>Date and Time: <input type="datetime-local" name="to" id="to" ></p>
                <p>Event: <input type="text" name="txt" id="txt" ></p>
                <button type="submit" class="button" name="submit">Set Past Event</button><br><br>
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST") {        
            if (isset($_POST['viewtable'])) {
                ...
                if (empty($event) || empty($date)) {
                    echo "<script>alert('Please enter value into the input.');</script>";
                } else {
                    $sql = "INSERT INTO pasttb (Email, Date, Event) VALUES ('$email','$date', '$event')";
        ...
        function reload_page($servername, $dbname, $username, $password, $email) {
            try {
                ...
                $stmt = $con->query("SELECT * FROM pasttb WHERE Email='$email' ORDER BY ID DESC");
            ...
            <h1 align="center">Past Event Record</h1>
                <table class="table">
                    <thead>
                        <tr>
                            <th>Event</th>
                    ...
                    <tbody>
                        ...
                        foreach ($rows as $row) : ?>
                            <tr>
                                ...
                                <td><a href="deletepastrec.php?ID=<?php echo $row['ID']; ?>" onclick="return confirm('Are you sure you want to delete this record')"><i class="fas fa-trash"></i></a></td>
                            </tr>
                            <script>
                                function updateInterval_<?php echo $row['ID']; ?>(eventDate) {
                                    var fromDate = new Date(eventDate);
                                    var toDate = new Date();
